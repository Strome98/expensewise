...
import { BarChart, Bar, XAxis, YAxis, Tooltip, Legend, ResponsiveContainer, LabelList, CartesianGrid, Cell } from 'recharts';
...
export default function CategoryChart({data, sortBy='income', limit=12}){
  ...
  const rows = useMemo(()=>{
    const merged = Object.values(data.reduce((acc, item)=>{
    ...
    const enriched = merged.map(m=>({
      ...
      income: m.income,
      expense: m.expense,
      incomePct: (grandIncome && !singleIncome) ? (m.income / grandIncome * 100) : 0,
      expensePct: (grandExpense && !singleExpense) ? (m.expense / grandExpense * 100) : 0,
    ...
    const sorter = (a,b)=> {
      switch(sortBy){
        ...
        case 'incomePct': return b.incomePct - a.incomePct;
        case 'expensePct': return b.expensePct - a.expensePct;
        case 'income':
  ...
  return (
    <div className="h-[480px] bg-white border rounded shadow p-4">
      ...
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={rows} margin={{ top: 20, right: 25, left: 0, bottom: 60 }}>
          <CartesianGrid strokeDasharray="3 3" />
          <XAxis dataKey="category" angle={-35} textAnchor="end" interval={0} height={70} />
          <YAxis />
          <Tooltip contentStyle={{ fontSize: '0.75rem' }} formatter={(value, name)=>{
          ...
          <Bar dataKey="income" name="Income" fill={COLOR_INCOME} radius={[4,4,0,0]}>
            <LabelList dataKey="incomePct" position="top" formatter={(v)=> v>0? v.toFixed(0)+'%':''} style={{ fontSize:'0.65rem' }} />
          </Bar>
          <Bar dataKey="expense" name="Expense" fill={COLOR_EXPENSE} radius={[4,4,0,0]}>
            <LabelList dataKey="expensePct" position="top" formatter={(v)=> v>0? v.toFixed(0)+'%':''} style={{ fontSize:'0.65rem' }} />
          </Bar>
          <Bar dataKey="net" name="Net" radius={[4,4,0,0]}>
            {rows.map(r=> <Cell key={r.category} fill={r.net>=0? COLOR_NET_POS: COLOR_NET_NEG} />)}
            <LabelList dataKey="net" position="top" formatter={(v)=> v!==0? formatHUF(v):''} style={{ fontSize:'0.65rem' }} />
          </Bar>
        </BarChart>
      </ResponsiveContainer>
      <p className="text-xs text-gray-500 mt-2">Labels show % share (income/expense). Net bar color: blue for positive (surplus), purple for negative (deficit).</p>
    </div>
  );
}
