  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-4">
        ...
        <button onClick={()=>setShowForm(f=>!f)} className="bg-blue-600 text-white px-3 py-1">{showForm? 'Close':'Add Transaction'}</button>
      </div>
      {showForm && <TransactionForm onDone={()=>setShowForm(false)} />}
      <TotalsHeader />
      {preferences?.alertOnNegativeNet && typeof totals?.net === 'number' && totals.net < 0 && (
      ...
      <div className="mt-4"><TransactionsTable /></div>
    </div>
  );
}
