...
export default function Profile() {
  ...
  async function requestDeletion() {
    ...
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/profile/delete/request`,
  ...
  async function cancelDeletion() {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/profile/delete/cancel`,
        { method: "POST", headers: { Authorization: `Bearer ${token}` } }
      );
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Failed");
  ...
  async function executeDeletion() {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/profile/delete/execute`,
        ...
        <div className="space-y-3">
          <div className="flex flex-col gap-1">
            ...
            <input
              ...
              className="border px-2 py-1 text-sm dark:bg-slate-700 dark:border-slate-600 dark:text-slate-100"
          ...
          <label className="flex items-center gap-2 text-sm">
            <input
              type="checkbox"
              checked={preferences.alertOnNegativeNet}
      ...
      <div className="p-4 bg-white dark:bg-slate-800 border dark:border-slate-700 rounded space-y-3 max-w-md">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="px-3 py-1 text-sm border rounded hover:bg-blue-50 dark:hover:bg-slate-700"
          >
            Export JSON
          </button>
          <button
