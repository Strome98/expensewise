...
export function AuthProvider({children}){
  ...
  const [preferences, setPreferences] = useState({ alertOnNegativeNet: true, currency: 'HUF' });
  ...
  setPreferences(res.data.preferences || { alertOnNegativeNet: true, currency: 'HUF' });
  ...
  async function updatePreferences(partial){
    try {
      ...
      if(typeof res.data.displayName === 'string') setDisplayName(res.data.displayName);
    } catch (e) {
  ...
  return <AuthContext.Provider value={{token, login, logout, preferences, displayName, updatePreferences, profileLoading, profileError}}>{children}</AuthContext.Provider>;
}
